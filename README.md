...
A-SOUL 粉丝自动应援工具 — 每天自动帮你保持嘉然、贝拉、乃琳、心宜、思诺的粉丝牌点亮，再也不怕牌子熄灭。
...
| 功能 | 默认 | 说明 |
|------|------|------|
| 🏅 粉丝牌点亮 | ✅ 开启 | 自动佩戴粉丝牌 → 发 10 条弹幕点亮牌子，保持 3 天可见 |
| 👍 视频点赞 | ❌ 关闭 | 给成员最近发布的视频自动点赞 |
| 💬 动态点赞 | ❌ 关闭 | 给成员最近发布的动态自动点赞 |

### 关于粉丝牌点亮

B 站规则：**3 天内没有在直播间完成任何任务，粉丝牌就会熄灭。** 熄灭后不掉等级、不减亲密度，但佩戴后别人看不到你的牌子。

点亮方式之一是在直播间发送 10 条弹幕。本工具每天自动帮你完成这件事，让 5 个成员的牌子永远不熄灭。
...
3. 等大约 2-3 分钟（每个成员发 10 条弹幕需要一些时间），页面会出现一条新的运行记录
4. 点进去，展开 **🏅 粉丝牌点亮** 那一步
5. 如果看到 `✅ 嘉然 ... 已点亮` 之类的输出，就说明一切正常！

之后每天北京时间 10:30 会自动执行。

## 可选功能：开启视频/动态点赞

默认只做粉丝牌点亮。如果你还想自动给成员的新视频和动态点赞：
...
- 所有代码完全开源，你可以自行检查每一行
- 只做弹幕和点赞操作，不会修改账号设置，不会发私信，不会关注陌生人
...
**Q: 怎么知道每天有没有成功？**
不用主动去看。只有失败时 GitHub 才会发邮件通知你。没收到邮件 = 一切正常。你也可以随时进 Actions 标签查看历史记录。

**Q: 可以改执行时间吗？**
打开 `.github/workflows/daily.yml`，修改 `cron: '30 2 * * *'` 中的数字。注意这里用的是 UTC 时间，北京时间需要减 8 小时。例如想改成下午 3 点执行，就写 `cron: '0 7 * * *'`。

**Q: 弹幕会很刷屏吗？**
工具内置了 15 条不同的日常弹幕（签到、打卡、加油、冲冲冲等），每次随机选取，不会重复发同一条。每条弹幕之间间隔 3 秒，不会触发限流。

**Q: 这个工具能帮我涨亲密度吗？**
发弹幕和点赞只能**点亮粉丝牌**，不能增加亲密度。涨亲密度需要观看直播（每天上限 30）或投币（1 币 = 10 亲密度），这些暂不支持自动化。

**Q: 投币会消耗硬币吗？**
默认不投币。本工具不会消耗你的硬币。

**Q: 操作间隔可以调吗？**
打开 `.github/workflows/daily.yml`，修改 `DANMAKU_DELAY` 调整同一直播间内弹幕间隔（默认 3 秒），修改 `DELAY_SECONDS` 调整成员之间的间隔（默认 8 秒）。
