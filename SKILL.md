---
name: asoul-support
description: "A-SOUL 粉丝应援工具 — 自动佩戴粉丝牌+发弹幕点亮牌子、视频点赞/投币/收藏、动态点赞。触发词：A-SOUL、asoul、签到、点赞、三连、应援、动态、点亮、粉丝牌、嘉然、贝拉、乃琳、心宜、思诺。"
---

# A-SOUL Support

A-SOUL 粉丝自动应援工具 — 粉丝牌点亮 + 视频互动 + 动态点赞，一句话搞定。
...
自动佩戴对应成员的粉丝牌，然后发送 10 条弹幕点亮牌子（保持 3 天可见）。
弹幕内容从内置消息池随机选取，也可自定义。

```bash
python3 {baseDir}/scripts/checkin.py
python3 {baseDir}/scripts/checkin.py --count 10
python3 {baseDir}/scripts/checkin.py --members 嘉然,贝拉
python3 {baseDir}/scripts/checkin.py --msg 签到 --msg 加油 --msg 冲冲冲
